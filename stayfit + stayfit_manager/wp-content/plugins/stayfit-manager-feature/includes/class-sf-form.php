    ...
    protected function createForm( $form_structure ){
        ...
        foreach( $form_structure["groups"] as $group_name => $properties ){
            ...
            if( ! empty( $properties["description"] ) ){
                $title_pattern = sprintf( '<p>%s</p>', $properties["description"] );
                if( ! empty( $properties['description_pattern'] ) ){
                    $title_pattern = sprintf( $properties['description_pattern'], $group_name );
                }
                $content .= sprintf( '<div class="col-xs-12 p-0 mb-2">%s</div>', $title_pattern );
            }
            
            if( ! empty( $this->view_options['group_class'] ) || ! empty( $this->view_options['group_style'] ) ){
                $class = ! empty( $this->view_options['group_class']  ) ? ' class="' . join(' ', $this->view_options['group_class']) . '" ' : '';
                $style = ! empty( $this->view_options['group_style']  ) ? ' style="' . $this->view_options['group_style'] . '" ' : '';
