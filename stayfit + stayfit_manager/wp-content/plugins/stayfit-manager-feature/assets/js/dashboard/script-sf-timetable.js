(function($){
    ...
    */
    ...
    sf_user_email_validation = function( email ){
        var result = /^[a-zA-Z0-9._-]+\@[a-zA-Z0-0.-]+\.[a-zA-Z]{2,5}$/;
    ...
    $('button[name="sf_dashboard_client-manager-creator[generate]"]').on( 'click', function( e ){
        ...
        var user_email = $('input[name="sf_dashboard_client-manager-creator[email]"]');

        if( user_email.val() == '' ){
            alert( "Polę adres email jest puste, zioom.\nPotrzebne jest do wygenerowania loginu klienta." );
            return;
        }

        if( ! sf_user_email_validation( user_email.val() ) ){
            alert( 'Wpisany adres email jest niepoprawny.' );
            return;
        }

        var user_login = user_email.val().replace('@', '.');

        $('input[name="sf_dashboard_client-manager-creator[login][rand_username]"]')
          .val( user_login )
          .removeAttr( "disabled");
