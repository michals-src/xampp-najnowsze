...
function bard_setup() {
	...
	register_nav_menus( array(
		...
		'primary' 		=> __( 'Main Menu', 'bard' ),
		'footer' 	=> __( 'Footer Menu', 'bard' ),
		'umenu' 	=> 'Nawigacja uzytkownika'
	) );
...
function bard_scripts() {
	...
	wp_enqueue_style( 'stayfit-repo-style', get_theme_file_uri( '/assets/css/repo.css' ), array(), '1.1' );

	wp_enqueue_style( 'stayfit-style', get_theme_file_uri( '/assets/css/theme.css' ), array(), '1.1' );

	// Theme Stylesheet
	wp_enqueue_style( 'bard-style', get_stylesheet_uri(), array(), '1.5' );
	...
	// Enqueue Custom Scripts
	wp_enqueue_script( 'ionicons', 'https://unpkg.com/ionicons@4.5.10-0/dist/ionicons.js', array( 'jquery' ), false, true );

	// Enqueue Custom Scripts
	//wp_enqueue_script( 'enedScroll', get_theme_file_uri( '/assets/js/enedScroll.js' ), array( 'jquery' ), false, true );
	// Enqueue Custom Scripts
...
//add_image_size( 'bard-page-thumbnail', 400, 240, true );
	...
	<div class="col-4 text-left">
		...
		<?php if ( bard_options( 'social_media_url_1' ) !== '' ) : ?>
			<a href="<?php echo esc_url( bard_options( 'social_media_url_1' ) ); ?>" target="<?php echo esc_attr($social_window); ?>">
				<span class="<?php echo esc_attr( $class ); ?>-icon" style="font-size:14px;color:#888888;"><?php bard_social_media_icon( '1' ); ?></span>
				<?php if ( $title ) : ?>
		...
		<nav style="display: inline-block;margin-left: 10px;">
			<ul>
				<li><ion-icon name="call" style="font-size: 16px;margin-right:8px;vertical-align:middle;"></ion-icon> <span style="letter-spacing: 1px;">Zadzwoń 100 200 300</span></li>
			</ul>
		</nav>
...
function theme_post_date($date){
	...
	$d = date('d', $strtime);
	$m = ucfirst(date('M', $strtime));
	$y = date('Y', $strtime);
	$m = str_replace(
		array('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul','Aug', 'Sep', 'Oct', 'Nov', 'Dec'),
		array('Sty', 'lut', 'Mar', 'Kwi', 'Maj', 'Cze', 'Lip', 'Sie', 'Wrze', 'Paź', 'Lis', 'Gru'),
		$m
	);
	return $d . ' ' . $m . ' ' . $y;
}
